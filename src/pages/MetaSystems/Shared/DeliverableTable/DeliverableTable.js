...
import { createDeliverableNote, deleteDeliverableNote, updateDeliverableNote } from 'redux/actions/metaSystem';
...
const DeliverableTable = ({ deliverables = [], mainSystem, systemTrend = {}, departments = [], users = [], onRowChange = noop }) => {
  ...
  const handleNoteDelete = (noteId) => {
    dispatch(deleteDeliverableNote({ noteId, mainId: mainSystem, _id: editData._id }));
  };

  const handleNoteClose = () => {
    setToggledNoteDialog(false);
    setTimeout(() => setEditIndex(-1), 200);
  };
    ...
    <Card>
      ...
      <CardContent>
        ...
        <NoteDialog
          ...
          notes={deliverables?.[editIndex]?.notes}
        />
